...
const onMiraieStateChanged = function (topic, payload) {
    ...
    const device = _devices.find(
      (d) => d.statusTopic === topic || d.connectionStatusTopic === topic
    );
    if (!device) return;
...
BrokerBridge.prototype.initialize = function (home, settings) {
    ...
    const deviceTopics = home.devices.map((d) => [
      d.statusTopic,
      d.connectionStatusTopic,
    ]);
  ...
  BrokerBridge.prototype.publishDeviceStatus = function (devices)  {
    devices.map((device) => {
      console.log(
        `publishing status for ${device.friendlyName}: ${device.status}`
      );
